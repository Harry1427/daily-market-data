...
for name, symbol in tickers.items():
    try:
        # 주말을 고려해 최근 5일치 데이터를 불러옵니다.
        data = yf.Ticker(symbol).history(period="5d")
        
        # 유효한 데이터가 2개 미만이면 건너뜀
        if len(data) < 2: 
            continue
        ...
        # 에러가 나도 로봇이 멈추지 않고 다음 지표로 넘어가게 합니다.
